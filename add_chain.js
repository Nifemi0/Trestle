...
function providers() {
  return {
    new: new ethers.JsonRpcProvider(T.rpc, undefined, { staticNetwork: true }),
    bot: new ethers.JsonRpcProvider(EXISTING.bot.rpc, undefined, { staticNetwork: true }),
    arb: new ethers.JsonRpcProvider(EXISTING.arb.rpc, undefined, { staticNetwork: true }),
  };
}
...
(async () => {
  ...
  const P = providers();
  const botW = new ethers.Wallet(dep.privateKey, P.bot);
  const arbW = new ethers.Wallet(dep.privateKey, P.arb);
  ...
  console.log(`  BOT               ${ethers.formatEther(await P.bot.getBalance(dep.address))} tBOT`);
  console.log(`  Arb Sepolia       ${ethers.formatEther(await P.arb.getBalance(dep.address))} ETH`);
  ...
  tx = await synth.enrollRemoteRouter(EXISTING.bot.domain, b32(EXISTING.bot.router));
  await tx.wait(); console.log('  new -> BOT :', T.explorer + tx.hash);
  tx = await synth.enrollRemoteRouter(EXISTING.arb.domain, b32(EXISTING.arb.router));
  await tx.wait(); console.log('  new -> Arb :', T.explorer + tx.hash);

  const botR = new ethers.Contract(EXISTING.bot.router, art.HypERC20Collateral.abi, botW);
  tx = await botR.enrollRemoteRouter(T.domain, b32(await synth.getAddress()));
  await tx.wait(); console.log('  BOT -> new :', EXISTING.bot.explorer + tx.hash);

  const arbR = new ethers.Contract(EXISTING.arb.router, art.HypERC20.abi, arbW);
  tx = await arbR.enrollRemoteRouter(T.domain, b32(await synth.getAddress()));
  await tx.wait(); console.log('  Arb -> new :', EXISTING.arb.explorer + tx.hash);
